...
if (theme_get_setting('fastresponse_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('fastresponse_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'fastresponse') .'/css/tabs.css', 'theme', 'screen');
}
...
function fastresponse_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('fastresponse_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = fastresponse_id_safe('page-'. $path);
    $body_classes[] = fastresponse_id_safe('section-'. $section);
...
function fastresponse_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = fastresponse_id_safe('node-type-' . $vars['type']);
...
function fastresponse_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = fastresponse_id_safe('block-' . $vars['block']->module);
    $classes[] = fastresponse_id_safe('block-' . $vars['block']->region);
    $classes[] = fastresponse_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('fastresponse_block_editing') && user_access('administer blocks')) {
...
function fastresponse_preprocess_comment(&$vars, $hook) {
...
function fastresponse_menu_item_link($link) {
...
function fastresponse_menu_local_tasks() {
  $output = '';
...
function fastresponse_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = fastresponse_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function fastresponse_id_safe($string) {
...
function fastresponse_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
