...
function fastresponse_settings($saved_settings, $subtheme_defaults = array()) {

  // Get the default values from the .info file.
  $defaults = fastresponse_theme_get_default_settings('fastresponse');
  ...
  $form['fastresponse_block_editing'] = array(
    ...
    '#default_value' => $settings['fastresponse_block_editing'],
    '#prefix'        => '<strong>' . t('Block Edit Links:') . '</strong>',
  );

  $form['themedev']['fastresponse_rebuild_registry'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Rebuild theme registry on every page.'),
    '#default_value' => $settings['fastresponse_rebuild_registry'],
    '#description'   => t('During theme development, it can be very useful to continuously <a href="!link">rebuild the theme registry</a>. WARNING: this is a huge performance penalty and must be turned off on production websites.', array('!link' => 'http://drupal.org/node/173880#theme-registry')),
    '#prefix'        => '<div id="div-fastresponse-registry"><strong>' . t('Theme registry:') . '</strong>',
    '#suffix'        => '</div>',
  );
...
function _fastresponse_theme(&$existing, $type, $theme, $path) {
  ...
  fastresponse_theme_get_default_settings($theme);
...
function fastresponse_theme_get_default_settings($theme) {
  $themes = list_themes();
